      ...
      >
        <div v-for="(presale, key) in this.allPresales" :key="key">
          <div class="p-6 rounded-lg static presale-blocks">
            <div class="rounded-lg w-48 fix-presale text-center">
            ...
            >
              ...
              <div
                :style="progressStyle"
                class="h-5 shadow-none flex flex-col text-center whitespace-nowrap text-white justify-center"
              >
  ...
  methods: {
    ...
    initCard: function () {
      const sweeterArray = this.allPresales.map(presale => {
       const test = presale.contributedEth/presale.hardcapInEth*100
          return test ;
      })
      ...
      for (i = 0; i < sweeterArray.length; i++) {
        // console.log(i);
        this.percentages.push(`width:` + sweeterArray[i] + `%; background-color: #059669;`);
          }
